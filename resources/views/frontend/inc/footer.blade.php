    ...
    <div class="row">
      <div class="col-lg-5 col-xl-4 text-center text-md-left">
        <div class="text-center text-md-left mt-4">
          ...
          <a href="/pdf.html" class="btn btn-primary btn-sm shadow-md"
            style="background-color: #2846b7;border-color: #2846b7;">
            เกี่ยวกับฉัน
          </a>
        </div>
        <div class="mt-4">
          <a href="{{ route('home') }}" class="d-block">
            ...
            @else
              <img class="lazyload" src="{{ static_asset('assets/img/placeholder-rect.jpg') }}"
                data-src="{{ static_asset('assets/img/logo.png') }}" alt="{{ env('APP_NAME') }}" height="44">
            @endif
          </a>
          <div class="my-3">
          ...
          <ul class="list-unstyled">
            ...
            @if (is_array($widget_one_labels))
              @foreach ($widget_one_labels as $key => $value)
                <li class="mb-2">
                  <a href="{{ is_array($widget_one_links) && isset($widget_one_links[$key]) ? $widget_one_links[$key] : '#' }}"
      ...
      <a href="{{ route('all-notifications') }}" class="text-reset d-block text-center pb-2 pt-3">
        <span class="d-inline-block position-relative px-2">
          ...
          @if (Auth::check() && count(Auth::user()->unreadNotifications) > 0)
            <span class="badge badge-sm badge-dot badge-circle badge-primary position-absolute absolute-top-right"
